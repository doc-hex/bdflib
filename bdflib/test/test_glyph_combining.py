...
class TestBuildUnicodeDecompositions(unittest.TestCase):
	...
	def test_basic_functionality(self):
		...
		self.assertTrue("\N{LATIN CAPITAL LETTER A WITH GRAVE}" in res)

		# It should not have decompositions for complicated things like
		# superscripts.
		self.assertFalse("\N{SUPERSCRIPT TWO}" in res)
		...
		for decomposable in res:
			for char, combining_class in res[decomposable]:
				self.assertEqual(type(char), str)
	...
	def test_base_dotless_i_and_j(self):
		...
		self.assertEqual(components[0],
				("\N{LATIN SMALL LETTER DOTLESS I}",0))
		...
		self.assertEqual(components[0],
				("\N{LATIN SMALL LETTER DOTLESS J}",0))
...
class TestFontFiller(unittest.TestCase):
	...
	def setUp(self):
		...
		self.font.new_glyph_from_data("a", ["4", "8"], 0,0, 2,2, 3, ord('a'))
		self.font.new_glyph_from_data("b", ["8", "4"], 0,0, 2,2, 3, ord('b'))
	...
	def test_add_decomposable_glyphs_to_font(self):
		...
		self.assertTrue(ord('c') in self.font)
		self.assertTrue(ord('d') in self.font)
...
class TestFontFillerCombiningAbove(unittest.TestCase):
	...
	def _build_composing_above_font(self, set_cap_height=False):
		...
		decompositions = {
				...
				# accent.
				'J': [('O',0),
					('\N{COMBINING CARON}',glyph_combining.CC_A)],
				'j': [('o',0),
					('\N{COMBINING CARON}',glyph_combining.CC_A)],
				'\N{MACRON}': [(' ', 0),
					('\N{COMBINING MACRON}',glyph_combining.CC_A)],
